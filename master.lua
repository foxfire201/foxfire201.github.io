...
if game.PlaceId == 8260276694 then
    ...
    function functions.Other:SendNotification(title, text, duration)
        return services['StarterGui']:SetCore("SendNotification", {
            Title = title,
            Text = text,
            Duration = duration
        }) -- dunno why return? it doesnt return anything anyway, its just nice ig
    end
    ...
    spawn(function() -- combot
        while true do
            ...
            if functions.Characters:IsAlive(services['Players'].LocalPlayer) and flags['combat/bot'] then
                char = functions.Characters:GetNearEntity(80)
                if char then
                    if functions.Other:Magnitude_Y(char.HumanoidRootPart.Position, services['Players'].LocalPlayer.Character.HumanoidRootPart.Position) <= 8 then
                        services['Players'].LocalPlayer.Character.Humanoid:MoveTo(char.HumanoidRootPart.Position)
                    end
                end
            end
            wait()
